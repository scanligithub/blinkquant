            ...
            async with httpx.AsyncClient(headers=headers, follow_redirects=True, timeout=60.0) as client:
                for fname in data_files:
                    ...
                    if "stock_list.parquet" in fname:
                        ...
                        # P2-MEM: Column pruning — only read needed columns to reduce peak RSS
                        # First read schema to check available columns
                        schema = pl.read_parquet_schema(bio)
                        available_cols = [c for c in KEEP_COLS if c in schema]
                        bio.seek(0)
                        df = pl.read_parquet(bio, columns=available_cols)
