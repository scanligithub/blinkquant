...
class TestHotJITWarmUp(unittest.TestCase):
    """P2-3A: HotJIT 必须在 warm-up 行为 null 时仍能挂载 rolling 指标。"""
    ...
    def test_hotjit_mounts_ma20(self):
        """MA(CLOSE,20) 前 19 行返回 null，HotJIT 不应因此跳过挂载。"""
        rows = []
        base = datetime.date(2026, 8, 3)
        for i in range(30):
            d = base + datetime.timedelta(days=i)
            rows.append((d, "sh.A", 10 + i))
        self._load(rows)

        col_name = "MA_CLOSE_20"
        df = data_manager.df_daily
        self.assertNotIn(col_name, df.columns)

        selection_engine._prepare_hot_jit("MA(CLOSE, 20)")

        df_after = data_manager.df_daily
        self.assertIn(col_name, df_after.columns,
                       f"HotJIT should mount {col_name} even though first 19 rows are null")

    def test_hotjit_mounts_ma5(self):
        """MA(CLOSE,5) 前 4 行返回 null，HotJIT 不应因此跳过挂载。"""
        rows = []
        base = datetime.date(2026, 8, 3)
        for i in range(15):
            d = base + datetime.timedelta(days=i)
        ...
        col_name = "MA_CLOSE_5"
        self.assertNotIn(col_name, data_manager.df_daily.columns)

        selection_engine._prepare_hot_jit("MA(CLOSE, 5)")

        self.assertIn(col_name, data_manager.df_daily.columns,
                       f"HotJIT should mount {col_name}")

    def test_hotjit_skips_truly_all_null(self):
        """如果表达式确实全 null（无数据），HotJIT 应跳过。"""
        rows = []
        base = datetime.date(2026, 8, 3)
        for i in range(5):
            d = base + datetime.timedelta(days=i)
        ...
        col_name = "MA_CLOSE_60"
        self.assertNotIn(col_name, data_manager.df_daily.columns)

        selection_engine._prepare_hot_jit("MA(CLOSE, 60)")

        self.assertNotIn(col_name, data_manager.df_daily.columns,
                          "HotJIT should skip when expression is always null")
