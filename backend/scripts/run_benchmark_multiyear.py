"""Multi-year benchmark harness for BlinkQuant (B3/B4).

Usage:
    python backend/scripts/run_benchmark_multiyear.py --start 2019 --end 2024 --output benchmarks/B3_2019_2024.json
    python backend/scripts/run_benchmark_multiyear.py --start 2010 --end 2024 --output benchmarks/B4_2010_2024.json
"""
...
def get_rss_mb():
    """Get current process RSS in MB (Windows-compatible)."""
    try:
        import psutil
        return psutil.Process().memory_info().rss / 1024 / 1024
    except ImportError:
        # Fallback: use tracemalloc or resource module
        try:
            import resource
            # resource.getrusage returns maxrss in KB on Linux, bytes on some systems
...
def load_year_data(year, hf_repo, token):
    """Download and load one year's data."""
    ...
    # No node filtering for benchmark (use all data)
    return df


def run_benchmark(start_year, end_year, top_n=20, cash=10_000_000):
    """Run multi-year benchmark with per-year memory tracking."""
    start_date = datetime.date(start_year, 1, 2)
    end_signal_date = datetime.date(end_year, 12, 31)

    _log(f"Benchmark: {start_year}-{end_year}")
    _log(f"Period: {start_date} .. {end_signal_date}")

    # Phase 1: Load data year by year, track memory per year
    ...
    for year in range(start_year, end_year + 2):  # +1 for T+1 lookahead
        ...
        try:
            df = load_year_data(year, REPO, TOKEN)
            parts.append(df)
            year_time = time.time() - year_start

            # Track memory after loading each year
            rss_mb = get_rss_mb()
...
def main():
    parser = argparse.ArgumentParser(description="Multi-year benchmark")
    ...
    _log(f"Benchmark: {args.start}-{args.end} â†?{args.output}")

    benchmark = run_benchmark(args.start, args.end, args.top_n, args.cash)
