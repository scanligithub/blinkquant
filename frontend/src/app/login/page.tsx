  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-slate-50 text-slate-900 font-sans">
      <div className="w-full max-w-sm bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
        <h1 className="text-2xl font-black bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent mb-1">BlinkQuant</h1>
        <p className="text-slate-500 text-sm mb-6">登录以继续使用</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {loading ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : '登录'}
          </button>
        </form>

        <p className="mt-6 text-sm text-slate-500 text-center">
          还没有账号？<Link href="/register" className="text-blue-600 font-semibold hover:underline">立即注册</Link>
        </p>
      </div>
    </main>
  );
