...
useEffect(() => {
  ...
  const fetchStatus = async () => {
    try {
      ...
      if (res.ok) {
        const json = await res.json();
        // Convert queueStats strings to numbers
        setClusterStatus({
          nodes: json.nodes || [],
          queueStats: {
            pending_selection: parseInt(json.queueStats?.pending_selection) || 0,
            pending_backtest: parseInt(json.queueStats?.pending_backtest) || 0,
            running: parseInt(json.queueStats?.running) || 0,
          },
          cluster_health: json.nodes?.every((n: any) => n.status === 'idle' || n.status === 'running') ? 'healthy' : 'degraded',
        });
      }
