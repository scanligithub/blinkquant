...
# Normalize line endings to LF for searching
...
new = b"""            <div className="flex flex-col gap-3">
            <div className="flex justify-center">
              <div className="text-xs md:text-sm font-mono text-slate-400 bg-white px-3 py-2 rounded-lg border shadow-sm">
...
if old in content:
    ...
    print('NOT FOUND in normalized')
    # Try with original line endings
    if old in content:
        content = content.replace(old, new)
        with open('src/app/page.tsx', 'wb') as f:
            f.write(content)
        ...
        print('NOT FOUND in original either')
        # Find the section
