with open(r'E:\数据中台\blinkquant\frontend\src\app\page.tsx', 'rb') as f:
    content = f.read()

# Find the section to replace
old_section = b'''            <div className="flex flex-col gap-3">
                ...
                <div key={idx} className={`text-xs md:text-sm font-mono px-3 py-2 rounded-lg border shadow-sm ${node.online ? \'bg-white border-slate-200\' : \'bg-red-50 border-red-200\'}`}>
                  ...
                  {node.online ? (
                    <div className="mt-2 space-y-1">
                      ...
                      <div className="flex justify-between gap-3">
                        <span className="text-slate-500 text-xs md:text-sm">\xe7\xb3\xbb\xe7\xbb\x9f\xe7\x9a\x84\xe7\xa9\xba\xe9\x97\xb2</span>
                        <div className="font-mono font-bold text-blue-600 text-right text-xs md:text-sm">{node.system_memory_free_gb} GB</div>
                      </div>
                      <div className="flex justify-between gap-3">
                        <span className="text-slate-500 text-xs md:text-sm">\xe7\xa3\x85\xe7\x9b\x98\xe7\xa9\xba\xe9\x97\xb2</span>
...
new_section = b'''            <div className="flex flex-col gap-3">
            <div className="flex justify-center">
              <div className="text-xs md:text-sm font-mono text-slate-400 bg-white px-3 py-2 rounded-lg border shadow-sm">
                ...
                >
                  ...
                  {node.status === \'running\' && node.task_type && (
                    <div className="mt-2 space-y-1 text-xs">
                      ...
                      {node.current_task_id && <span>\xe4\xbb\xbb\xe5\x8a\xa1ID: #{node.current_task_id}</span>}
                    </div>
                  )}
                </div>
              ))}
...
if old_section in content:
    ...
    with open(r'E:\数据中台\blinkquant\frontend\src\app\page.tsx', 'wb') as f:
        f.write(content)
    print('Replaced successfully')
else:
    print('NOT FOUND - searching for partial match')
    # Find partial matches
    for i in range(0, len(old_section), 200):
        chunk = old_section[i:i+200]
        pos = content.find(chunk)
        if pos != -1:
            print(f'Found chunk at {pos}: {chunk[:50]}...')
        else:
            print(f'Chunk {i//200} not found')