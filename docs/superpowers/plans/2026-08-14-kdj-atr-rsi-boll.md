...
- [ ] **Step 1: 更新 `test_builtin_indicators_present` 期望 21 个算子**
...
- [ ] **Step 2: 新增 `test_new_indicators_signatures`**
...
- [ ] **Step 2: 在 `INDICATORS` 字典末尾（`BARSLAST` 条目后，L54-55 之间）追加 6 条目**
...
- [ ] **Step 3: 更新 `DESCRIPTIONS`（L75-82 追加 6 条）**
...
- [ ] **Step 4: 更新 `EXAMPLE_QUERIES`（追加 2 条）**
...
Run: `python tests/test_registry.py`
Expected: ALL PASS（`test_builtin_indicators_present` 21 算子、`test_new_indicators_signatures`、既有 window/描述/unit 测试全部通过）
...
- [ ] **Step 1: 添加测试类**（在 `TestSignatureRecursion` 之后、`if __name__` 块之前，约 L242 前）
...
Run: `python tests/test_security.py`
Expected: ALL PASS（含既有 34 项 + 新增 11 项）
...
- [ ] **Step 2: 同步修改测试复制版 `isSeriesExpr`（frontend/tests/select-nl.test.mjs L159-190）**
...
- [ ] **Step 1: 后端全量**（确认 registry + security 一起绿）
...
- [ ] **Step 4: 提交测试更新例（示例 queries 同步至前后端一致）**

若 Step 1-2 全绿，无额外代码改动，跳过提交（已在 Task 2/5/6 分别提交）。否则修复并提交。

### 验收对照

- [ ] spec「验收标准 1」：backend 两个测试文件全绿
- [ ] spec「验收标准 2」：前端测试 + tsc 全绿
- [ ] spec「验收标准 3」：`CROSS_UP(KDJ_K(9,3), KDJ_D(9,3))` 后端可解析、前端 validateFormula 通过
- [ ] spec「验收标准 4」：`nl_meta()` 返回 21 个算子（由 Task 2 的 `test_new_indicators_signatures` + `test_nl_meta_shape` 锁住）
---
